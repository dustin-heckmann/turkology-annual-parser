...
class ElasticSearchRepository(object):
    ...
    def __create_index(self):
        try:
            self._es.indices.delete('citations')
        except NotFoundError:
            pass
        self.delete_all_data()
        with open(join(dirname(__file__), 'index_settings.json')) as settings_file:
            self._es.indices.create(index='citations', body=json.load(settings_file))
    ...
    def insert_citations(self, citations):
        citations = map(lambda citation: {
            '_index': 'citations',
            '_type': 'citation',
            **self.__citation_as_dict(citation)
        }, citations)
        bulk(self._es, citations, chunk_size=70000, request_timeout=200)

    @staticmethod
    def __citation_as_dict(citation: Citation):
        return asdict(
