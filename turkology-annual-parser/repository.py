...
class MongoRepository(object):
    ...
    def find_citations(self, query=None, limit=0, skip=0, order_fields=None, fullyParsed=None, obsolete=False):
        if query is None:
            criteria = {}
        else:
    ...
    def insert_citation(self, citation):
        if '_id' in citation:
            self.citations.update({'_id': citation['_id']}, {'$set': {'_obsolete': True}})
            del citation['_id']
        self.citations.update({'volume': citation['volume'], 'number': citation['number']},
                              {'$set': {'_obsolete': True}})
