...
class CitationParser(object):
    ...
    series_pattern = re.compile(
        '{{{ (?:numberOfPages|material|datePublished) }}}([ .,]*\(([^)]+)\))\. *?(?:$|{{{ comment)')
    ...
    def parse_citation(cls, citation):
        ...
        if review_match:
            ...
            if review_type == 'Rez.':
                field_name = 'reviews'
                citation[field_name] = re.split('\s*—\s*', review_match.group(2))
        ...
        if comment_match:
            ...
            if ta_references_match:
                comment_field_name = 'taReferences'
            else:
                comment_field_name = 'comment'
            citation[comment_field_name] = comment
        ...
        if match:
            ...
            text = text[:match.span()[0]] + ' {{{ numberOfVolumes }}} {{{ location }}} {{{ datePublished }}} ' + text[
                                                                                                                 match.span()[
                                                                                                                     1]:]
        ...
        if loc_year_pages_match:
            ...
            text = text[:loc_year_pages_match.span()[0]] + loc_year_pages_match.group(
                1) + ' {{{ location }}} {{{ datePublished }}} {{{ numberOfPages }}}' + text[
                                                                                       loc_year_pages_match.span()[1]:]
        ...
        if role_person_match:
            ...
            text = text[:role_person_match.span(1)[0]] + (' {{{ %s }}} ' % role_name) + text[
                                                                                        role_person_match.span()[1]:]
